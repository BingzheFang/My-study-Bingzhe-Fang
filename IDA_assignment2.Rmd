...
# Question 4
In this case, we set y_min = {y_i}_{i=1}^{m} and y_obs = {y_i}_{i=m+1}^{n}, the complete data likelyhood is
$$
...
Therefore
\begin{align*}
Q(\theta|\theta^{(t)})&= \sum_{i=1}^{m}{p_i(\beta)\log(p_i(\beta))}\\
&+ \sum_{i=1}^{m}{((1-p_i(\beta^{(t)}))\log(1-p_i(\beta)))}\\
&+ \sum_{i=m+1}^{n}{(y_i\log(p_i(\beta^{(t)}))+(1-y_i)\log(1-p_i(\beta)))}
\end{align*}
